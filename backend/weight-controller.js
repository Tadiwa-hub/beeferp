...
export const getWeightRecords = async (req, res) => {
  ...
  try {
    // Validate that the animal belongs to the user
    const animal = await getOne('SELECT id FROM animals WHERE id = $1 AND created_by = $2', [animalId, userId]);
    if (!animal) {
      return res.status(404).json({ error: 'Not Found', message: 'Animal not found or unauthorized' });
    }
...
export const createBulkWeightRecords = async (req, res) => {
  ...
  try {
    ...
    for (const record of records) {
      ...
      // Verify animal exists
      const animal = await getOne('SELECT id FROM animals WHERE id = $1 AND created_by = $2', [animal_id, userId]);
      if (!animal) {
        return res.status(404).json({
          error: 'Not Found',
          message: `Animal ${animal_id} not found or unauthorized`,
        });
      }
  ...
  try {
    const result = await query(
      `SELECT 
       ...
       WHERE a.id = $1 AND a.created_by = $3`,
      [animalId, days, req.user.id]
    );
  ...
  try {
    const result = await getAll(
      `SELECT 
       ...
       WHERE a.status = 'active' AND a.created_by = $2
       ORDER BY adg DESC`,
      [days, req.user.id]
    );
