...
export const getAnimals = async (req, res) => {
  ...
  try {
    ...
    const countResult = await query('SELECT COUNT(*) as total FROM animals');
...
export const updateAnimal = async (req, res) => {
  ...
  try {
    // Check animal exists
    const animal = await getOne('SELECT id FROM animals WHERE id = $1', [id]);
    if (!animal) {
      return res.status(404).json({
        error: 'Not Found',
        message: 'Animal not found',
    ...
    const result = await query(
      ...
      [breed, current_weight, target_weight, status, health_notes, id]
    );
